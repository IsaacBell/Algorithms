...
void solution() {
  ...
  fo(i,n) fo(j,2) Fo(fight, 1, min((ll) 3, n-i+1)) {
    // bool skip1 = !j && A[i];
    // bool skip2 = !j && i < n-1 && A[i+1];
    // ckmin(dp[i+1][j^1], dp[i][j] + j * hard);
    // ckmin(dp[i+2][j^1], dp[i][j] + j * hard);
    
    ll hard = A[i] + (fight > 1 ? A[i + 1] : 0);
    ckmin(dp[i+fight][j^1], dp[i][j] + j * hard);
  }

  put( min(dp[n][0], dp[n][1]) );
}
