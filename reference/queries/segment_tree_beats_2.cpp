...
void update_mod(ll i, ll lo, ll hi, ll a, ll b, ll x) {
  deb2(i, tree[i].p.mx1);
	if (hi <= a || b <= lo || tree[i].p.mx1 < x) return;
	if (lo >= a && b <= hi && tree[i].p.mx1 == tree[i].p.mn1) {
	  put("here");
    ll val = tree[i].p.mx1 % x;
    tree[i].sum = tree[i].p.mx1 = val;
  }
  ...
  shift(i);
	...
	update_mod(i << 1, lo, mid, a, b, x);
	update_mod(i << 1 | 1, mid+1, hi, a, b, x);
