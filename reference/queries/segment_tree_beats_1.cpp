...
vl tsum(MAXN * 4), tmax(MAXN * 4);

void update_mod(ll l, ll r, ll v, ll t = 1, ll tl = 1, ll tr = n) {
...
void update_set(ll i, ll v, ll t = 1, ll tl = 1, ll tr = n) {
	if (tl == tr) {
		tsum[t] = tmax[t] = v;
		return;
	}
...
ll query(ll l, ll r, ll t = 1, ll tl = 1, ll tr = n) {
	if (r < tl || tr < l) {
		return 0;
	}
	else if (l <= tl && tr <= r) {
