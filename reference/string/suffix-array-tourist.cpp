...
int main() {
  ...
  vector<unsigned long long> pw(len + 1);
  pw[0] = 1;
  
  for (int i = 0; i < len; i++) pw[i + 1] = modmul(pw[i], step);
  
  vector<unsigned long long> hh(len + 1);
  hh[0] = 0;
  for (int i = 0; i < len; i++) {
    hh[i + 1] = modmul(hh[i], step) + s[i];
    if (hh[i + 1] >= mod) hh[i + 1] -= mod;
  }
  ...
  auto get_hash = [&](int L, int R) {
    unsigned long long u = hh[R + 1] + mod - modmul(hh[L], pw[R - L + 1]);
    if (u >= mod) u -= mod;
    return u;
  };
...
//      if (get_lcp(from[i], from[i] + j) >= u - j) {
      if (get_hash(from[i], from[i] + (u - j) - 1) == get_hash(to[i] - (u - j) + 1, to[i])) {
        res[i] = 1;
        break;
      }
    }
  ...
  for (int i = 0; i < tt; i++) {
    ...
    for (int u = 1; u <= SQRT && u < to[i] - from[i] + 1; u++) {
      if (get_hash(from[i], from[i] + u - 1) == get_hash(to[i] - u + 1, to[i])) {
        res[i] = 2;
        break;
      }
    }
