...
TrieNode* insert(struct TrieNode* root, string key) {
  struct TrieNode* crawl = root;
  fo(i, key.sz()) {
    int idx = key[i] - 'a';
    crawl->ch[idx] = getNode();
    crawl = crawl->ch[idx];
  }
  crawl->isEnd = true;
}
...
TrieNode* search(struct TrieNode* root, string key) {
  ...
  fo(i, key.sz()) {
    int idx = key[i] - 'a';
    if (!crawl->ch[idx]) return NULL;
    crawl = crawl->ch[idx];
  }
  if (crawl->isEnd) return crawl;
}

void dfs(struct TrieNode* cur, string str, vstr& res) {
  fo(i, str.sz()) {
    if (!!cur->ch[i]) {
      string tmp = str;
      tmp.pb(i + 'a');
      dfs(cur->ch[i], tmp, res);
    }
  }
