...
void dfs2(vvl& adj, ll u,ll pa = -1,ll acc = 0) {
  (dp[u] *= (1 + acc)) %= m;
  vl buffer { acc };
  trav(nei,adj[u]) if (nei != pa) buffer.pb(dp[nei]);
  
  vl tail(buffer.sz(),1);
  ford1(i, buffer.sz())
      tail[i - 1] = tail[i] * (buffer[i] + 1) % m;

  ll j = 1;
  ll head = acc + 1;
  trav(nei, adj[u]) if (nei != pa) {
      dfs2(adj,nei,u,tail[j] * head % m);
      (head *= (buffer[j] + 1)) %= m;
      j++;
  }
}
