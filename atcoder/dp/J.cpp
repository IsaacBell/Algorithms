...
double build(ll x, ll y, ll z, ll n) {
  if(x<0 || y<0 || z<0){
    return 0;
  }
  if (!x && !y && !z)
  ...
  if (dp[x][y][z] > -0.9)
    return dp[x][y][z];
  double expe = n + x*build(x-1,y,z,n) + y*build(x+1,y-1,z,n) + z*build(x,y+1,z-1,n);
  return dp[x][y][z] = expe/(x+y+z);
}
...
void solution() {
  ...
  cout << fixed << setprecision(10) << build(hm[1], hm[2], hm[3], n) << nl;
}
