...
struct LCA {
  using vf = vector<F>;
  using vvf = vector<vf>;
  vf height, euler, first, segtree;
  vb visited;
  ll nn;

  LCA(vvf &adj, F root = 0) {
    nn = adj.sz();
    height.rsz(nn);
    first.rsz(nn);
    euler.reserve(nn * 2);
  ...
  void dfs(vvf &adj, F node, F h = 0) {
    visited[node] = true;
    height[node] = d[node] = h;
    first[node] = euler.sz();
    euler.pb(node);
  ...
  F query(F node, F b, F e, F L, F R) {
    if (b > R || e < L) return -1;
  ...
  F lca(F u, F v) {
    auto left = first[u], right = first[v];
    if (left > right) swap(left, right);
    return query(1, 0, euler.sz() - 1, left, right);
  }
};
#define F first
...
void solution() {
  ...
  while (q--) {
    rd(a >> b);
    ll parent = lca.lca(a,b);
    // deb(parent);
    // deb2(a,b);
    // deb(lca.height[parent]);
    // deb2(lca.height[a], lca.height[b]);
    put( lca.height[a] + lca.height[b] - (2*lca.height[parent]));
  }
}
