...
void solution() {
  ...
  ll cnt = 1;
  Fo(i,1,n) cnt += (pos[i] > pos[i+1]);

  set<pl> updates;
  while(m--) {
    rd(a >> b);
    if (A[a] < n) updates.insert({A[a], A[a] + 1});
    if (A[a] > 1) updates.insert({A[a] - 1, A[a]});
    if (A[b] < n) updates.insert({A[b], A[b] + 1});
    if (A[b] > 1) updates.insert({A[b] - 1, A[b]});

    trav(upd, updates) cnt -= (pos[upd.F] > pos[upd.S]);
