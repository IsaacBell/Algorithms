...
struct SubArrayProcessor {
  ...
  // Divide array into k subarrays w/ min possible max sum
  ...
  // # subarrays w/ at most k distinct values
  ll numSubarraysWithAtMostKDistinctVals(ll k) {
    // Two-Pointer
    // 
    // The moment you get greater than k distinct
    // elements add up to answer and then start again.
    // Keep doing this also to avoid overcounting make
    // a variable last which will store the last pointer
    // upto where you have already added to answer
    // 
    // https://codeforces.com/blog/entry/87320
    mll freq;
    ll distinct = 0; ll j = 0;
    ll total = n; ll last = 0;
    fo(i,n) {
        freq[data[i]]++;
        if(freq[data[i]] == 1) distinct++;
        if(distinct > k){
            ll len = i - j;
            ll b = last - j + 1;
...
void solution() {
  rd(n >> k);
  vl A(n); readall(A);
  SubArrayProcessor<ll> sp(A);

  put(sp.numSubarraysWithAtMostKDistinctVals(k));
}
