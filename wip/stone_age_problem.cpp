...
class SegmentTree {
  using vit = vector<InputType>;
  using Nodes = vector<SegmentTreeNode>;
	Nodes nodes;
	int N;
	
public:
	SegmentTree(vit& arr, int N_): N(N_) {
		nodes.rsz(N_);
		buildTree(arr, 1, 0, N_-1);
	}
...
private:
	void buildTree(vit& arr, int stIndex, int start, int end) {
...
void solution() {
  ...
  vl A(n);
  readall(A);
  SegmentTree<ll, ll, ll> tree(A, n);

  while (q--) {
    rd(w);
    if (w == 1) {
      ll idx, val; rd(idx >> val);
