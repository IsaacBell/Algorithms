...
void solution() {
  ...
  if (count(all(s), 'A') != a + ab + ba) {putNo;}
  ...
  ll common = 0;
  Fo(i,1,n+1) {
    // 2 consec letters, strs break here
    if (s[i] == s[i-1]){
      if (curr.sz() == 1) {
        // grp 1
        // pass
        ...
        // grp 3
        if (curr[0] == 'A') AB.pb(curr.sz() / 2);
        else BA.pb(curr.sz() / 2);
      }
  ...
  sortall(AB); sortall(BA);

  ll cntAB = 0, cntBA = 0;
  ll commonLeft = common;

  // try to get cntAB=ab from AB strs
  for (auto & x : AB) {
    ll req = ab - cntAB;
    if (req >= x) cntAB += x, x = 0;
    else cntBA += req, x -= req;
  }
  // try to get cntBA=ba from BA strs
  for (auto & x : BA) {
    ll req = ba - cntAB;
    if (req >= x) cntBA += x, x = 0;
    else cntBA += req, x -= req;
  }
  ...
  if (cntAB < c) {
    ll req = ab - cntAB;
    cntAB += min(req, commonLeft);
    commonLeft -= min(req,commonLeft);
    req = ab - cntAB;

    // if need to borrow from BA
    if (req > 0) {
      for (auto &x : BA) {
        if (!x) cont;
        ll req2 = ab - cntAB;
        if (req2 >= x-1) cntAB += x-1, x = 0;
        else cntAB += req2, x -= req2 + 1;
      }
    }
  }
  ...
  if (cntBA < c) {
    ll req = ba - cntBA;
    cntBA += min(req, commonLeft);
    commonLeft -= min(req, commonLeft);
    req = ba - cntBA;

    // if need to borrow from AB
    if (req > 0) {
      for (auto &x: AB) {
        if (!x) cont;
        ll req2 = ba - cntBA;
        if (req2 >= x-1) cntBA += x-1, x = 0;
        else cntBA += req2, x -= req2 + 1;
      }
    }
  }

  if (cntAB >= ab && cntBA >= ba) {putYes;}
  else {putNo;}
}
